...
def confirm_keyboard(*, confirm_cb: str, cancel_cb: str = "add:cancel") -> InlineKeyboardMarkup:
...
def edit_field_keyboard() -> InlineKeyboardMarkup:
    rows = [
        ...
        [InlineKeyboardButton(text="✏ Период", callback_data="edit:field:billing_period")],
        [
            InlineKeyboardButton(
                text="✏ Дата списания", callback_data="edit:field:next_billing_date"
            )
        ],
        [InlineKeyboardButton(text="✏ Способ оплаты", callback_data="edit:field:payment_method")],
        [InlineKeyboardButton(text="✏ Ссылка", callback_data="edit:field:management_url")],
...
def subscription_card_keyboard(subscription_id: int) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="✏ Изменить", callback_data=f"sub:{subscription_id}:edit")],
            [
                InlineKeyboardButton(
            ...
            [InlineKeyboardButton(text="🗑 Удалить", callback_data=f"sub:{subscription_id}:delete")],
...
def subscriptions_list_keyboard(
    ...
    for sub_id, label in items:
        rows.append([InlineKeyboardButton(text=label[:60], callback_data=f"sub:{sub_id}:open")])
    nav: list[InlineKeyboardButton] = []
    if page > 0:
        nav.append(InlineKeyboardButton(text="◀", callback_data=f"{filter_cb}:page={page - 1}"))
...
def status_filter_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(text="🟢 Активные", callback_data="list:filter:active"),
                InlineKeyboardButton(text="⏸ Приостановленные", callback_data="list:filter:paused"),
            ],
            [
                InlineKeyboardButton(text="❌ Отменённые", callback_data="list:filter:cancelled"),
                InlineKeyboardButton(text="📦 Архив", callback_data="list:filter:archived"),
            ],
